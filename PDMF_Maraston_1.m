...
fract_eve_WD = sum(1./sqrt(m(i_WD)));

fract_NS = length(i_NS);
fract_m_NS = sum(m(i_NS));
fract_eve_NS = sum(1./sqrt(m(i_NS)));

fract_BH = length(i_BH);
fract_m_BH = sum(m(i_BH));
fract_eve_BH = sum(1./(sqrt(m(i_BH))));
